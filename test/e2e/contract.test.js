...
describe('E2E nodejs', () => {
  ...
  test('Conversation', async () => {
    ...
    const code = new Uint8Array(
      fs.readFileSync(`${__dirname}/../../contract/contract.go`)
    );
    const [tx, txId] = client.createTransaction(
      ...
      '_Deployments',
      'deployService',
      [Orbs.argString('testContract'), Orbs.argUint32(1), Orbs.argBytes(code)]
    );
    ...
    const messageId = await c.sendMessageToChannel(
      'defaultChannel',
      "what's up?"
    );
